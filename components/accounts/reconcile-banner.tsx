...
export function ReconcileBanner({ due }: { due: AccountRow[] }) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 rounded-well bg-action-tint p-3 transition-colors hover:border-action/50"
    >
      <span className="flex size-9 shrink-0 items-center justify-center rounded-control bg-action text-white">
        <Bank size={18} />
      </span>
