> ⚠️ **سند بایگانی — دیگر معتبر نیست.**
> این خروجی فاز طراحی اول (لاجورد `#23459B`) است. در ۲۰۲۶-۰۹-۲۲ هویت بصری از نو طراحی شد («دفتر و مداد»).
> منبع درست توکن‌ها: بلوک `@theme` در `app/globals.css`. خلاصه‌ی تصمیم‌ها: بخش ۴ در `PLAN.md`.
> این فایل نگه داشته شده چون *دلیل* انتخاب‌های اولیه را ثبت می‌کند، نه چون وضعیت فعلی را توصیف می‌کند.
