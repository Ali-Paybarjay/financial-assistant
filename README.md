...
۹. **ثبت از هر صفحه‌ای، با یک نوار نه یک دکمه.** `components/entry/composer.tsx` ته هر صفحه است و تب‌بار داخل خودش می‌نشیند — یک نوار ثابت، نه دو تا. قبل از هر فراخوانی مدل، `lib/entry/quick-parse.ts` تصمیم می‌گیرد آیا چیزی برای خواندن هست: «قهوه ۵» فرم را پیش‌پر می‌کند و هیچ درخواستی نمی‌فرستد. سقف روزانه به تفکیک مسیر است (`FEATURE_DAILY_LIMITS`)، نه یک عدد مشترک.
