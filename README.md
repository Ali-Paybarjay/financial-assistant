...
۹. **RTL کامل.** فقط کلاس‌های منطقی (`ms-`/`me-`/`ps-`/`pe-`/`start-`/`end-`). `pnpm check:rtl` این را اجبار می‌کند.

۱۰. **زبان کد انگلیسی، زبان رابط کاربری فارسی.**

۱۱. **دو بخش، یک دفتر.** «حسابداری شخصی» و «دنگ و دونگ» هیچ صفحه و هیچ آیتم ناوبری‌ای با هم شریک نیستند؛ بخشِ جاری از روی آدرس خوانده می‌شود و هیچ‌جا ذخیره نمی‌ماند. تنها اتصالشان ردیفی است که «دنگ و دونگ» در `transactions` می‌نویسد، و آن را **تریگر** می‌نویسد نه action — با `dong_expense_id`/`dong_payment_id` به ردیف اصلی گره خورده، از سمت شخصی ویرایش و حذف نمی‌شود، و با حذفِ دوره خودش هم می‌رود. مبلغِ آینه همیشه کلِ فاکتور است، نه سهم کاربر: صورت‌حساب بانک هم همین را می‌گوید.

## ساختار

```
app/(auth)      ورود و ثبت‌نام
app/(app)/      بخش محافظت‌شده. صفحه‌ی `/` انتخاب بین دو بخش است:
                حسابداری شخصی (داشبورد، تراکنش‌ها، حساب‌ها، درآمد، اهداف، صورت‌حساب، تنظیمات)
                و دنگ و دونگ (دوره‌ها)
