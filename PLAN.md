...
| # | ریسک | اثر | کاهش |
|---|---|---|---|
| **R1** | **`MediaRecorder` روی iOS Safari** فقط `audio/mp4` می‌دهد، نه `audio/webm`؛ و در برخی نسخه‌ها رفتار ضبط ناپایدار است | تب «صدا» روی آیفون بی‌صدا شکست می‌خورد — و مخاطب اصلی محصول موبایل است | `MediaRecorder.isTypeSupported` را در زمان اجرا چک کن و mime را انتخاب کن؛ Whisper هر دو را می‌پذیرد. اگر هیچ‌کدام پشتیبانی نشد، تب صدا **پنهان** شود نه اینکه خطا بدهد. تست واقعی روی یک iPhone در M7، نه شبیه‌ساز |
| **R2** | **کیفیت Whisper روی فارسی محاوره‌ای** — «چهل و پنج دلار» یا «سیصد و بیست تومن» ممکن است اشتباه رونویسی شود | تراکنش با مبلغ غلط ثبت می‌شود | طراحی این را حل کرده: transcript **همیشه** پیش از parse نشان داده می‌شود و قابل ویرایش است. به‌علاوه قاعده‌ی «مبلغ را حدس نزن» در پرامپت: اگر مبلغ مبهم بود، reject با پیام فارسی |
