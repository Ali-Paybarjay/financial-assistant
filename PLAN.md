...
> **بازطراحی ۲۰۲۶-۰۹-۲۲ — «دفتر و مداد».** هویت بصری از نو ساخته شد. مرجع زنده و تنها منبع درست، بلوک `@theme` در `app/globals.css` است؛ یادداشت بالای همان فایل منطق هر انتخاب را توضیح می‌دهد. `design_handoff_financial_assistant/` سند فاز طراحی اول است و دیگر معتبر نیست.

خلاصه‌ی تصمیم‌ها:

### ۴.۱ رنگ

نام‌گذاری بر پایه‌ی **نقش** است نه رنگ، چون رنگ عوض می‌شود و نقش نه: `action #302C73` (نیلِ ایرانی — کاری که می‌شود انجام داد: لینک، دکمه‌ی اصلی، آیتم فعال ناوبری، فوکوس) · `ink #191A2E` (این یک واقعیت است) · `guess #8C5B14` (اخرایی — مدل این را حدس زده) · `positive #0F6B44` · `negative #A62A21` · `surface #FFFFFF` (ورق) · `paper #EDECF2` (زمینه‌ی زیر ورق).

قاعده‌ی سخت: رنگ فقط معنا می‌رساند، هرگز تزئین نیست. هیچ چیزی رنگی نمی‌شود چون خالی به‌نظر می‌رسید — به همین دلیل آیکون ردیف‌های لیست و نشانه‌های تزئینی به `ink-faint` رفتند.

هر جفت متن/زمینه روی هر دو `#ffffff` و `paper` اندازه‌گیری شد و WCAG AA را رد می‌کند؛ `hairline-strong` هم به `#8A89A1` تیره شد تا لبه‌ی ورودی‌ها ۳:۱ (WCAG 1.4.11) بدهد. پالت نمودار تک‌هیو و پنج‌پله از action، با `#C9C8D3` برای «سایر».

### ۴.۲ تایپوگرافی

بدنه Vazirmatn (۴۰۰/۵۰۰/۶۰۰/۷۰۰)، display Estedad (۷۰۰/۸۰۰)، هر دو SIL و self-host شده. تقسیم کار روشن است: **Vazirmatn گزارش می‌دهد، Estedad حرف می‌زند** — و Estedad حداکثر دو بار در هر صفحه دیده می‌شود: یک عدد که سؤال صفحه را جواب می‌دهد، و یک سطر که سؤال را می‌پرسد. مقیاس از `figure-hero` (clamp تا 44px) تا `micro 11px`.
...
یک دستور زبان، دو کار: **پیوسته یعنی قطعی‌شده، خط‌چین یعنی هنوز نه.**

۱. **زیر مقدارهای مدل** (`.rule-guess` / `.rule-settled`): نوار ۲px — خط‌چین اخرایی + واژه‌ی «حدس زدم» تا وقتی تأیید نشده، پیوسته‌ی جوهری بعد از تأیید (انیمیشن ۱۸۰ms). در سه لایه: کارت تأیید (per-field) · ردیف لیست تراکنش · عدد هزینه‌ی داشبورد + بنر «۲ تراکنش تأییدنشده در این جمع هست».

۲. **باندِ ماه** (`.runway`، زیر عدد مانده): روزهای سپری‌شده جوهریِ پیوسته، روزهای باقی‌مانده مدادیِ خط‌چین. «چقدر مانده» هیچ‌وقت سؤالی درباره‌ی یک عدد تنها نیست — ۴۰۰ دلار در ۲۸ام راحت است و در ۴ام نگران‌کننده. همان دستور زبان روی نوار پیشرفت هدف‌ها و گام‌های آنبوردینگ هم می‌نشیند: چیزی که هنوز نشده، قطعی نیست.
